...
export default class Storage {
  constructor(options: Object) {
    const { defaultExpires, enableCache } = options;
    //过期时间（单位毫秒），默认一天
    this.defaultExpires = typeof defaultExpires === 'number' ? defaultExpires : 1000 * 3600 * 24;
    //是否缓存，默认开启
  ...
  _getStorageData(key: string, defaultData = null) {
    return AsyncStorage.getItem(this._prefix + key).then(data => {
      if (data !== null) {
