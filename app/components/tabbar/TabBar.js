...
import { StyleSheet, Text, View, Animated, ViewPropTypes, Platform } from 'react-native';
...
export default class TabBar extends React.Component {
  ...
  setAnimationValue({ value }) {
    ...
    if (differ >= 2) {
      ...
      if (activeTab - this.props.fromIndex > 0) {
        //往右划
      ...
      } else if (activeTab - this.props.fromIndex < 0) {
        //往左划
        this.fonts.forEach((font, i) => {
          if (this.props.fromIndex === i) {
            const fontSize = minSize * (1 + progress) * ratio;
  ...
  _renderUnderline() {
    ...
    console.log(scaleX, 'bbbbbbbbbbbbbb');
    return (
      <Animated.View
        style={[
          tabUnderlineStyle,
  ...
  render() {
    ...
    let { containerWidth, tabs, tabList, scrollValue, backgroundColor } = this.props;
